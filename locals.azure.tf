...
locals {
  ...
  effective_runner_group_name = var.runner_existing_group_name != null ? "group: ${var.runner_existing_group_name}" : (local.use_runner_group ? "group: ${local.resource_names.runner_group_name}" : "self-hosted")
