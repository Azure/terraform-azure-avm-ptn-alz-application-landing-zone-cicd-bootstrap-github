...
variable "azure_existing_agents_subnet_resource_id" {
  type        = string
  default     = null
  description = "The resource ID of a pre-existing subnet for self-hosted runner compute (BYO mode). The subnet must have the appropriate delegation for the chosen `runner_compute_type`."
}
...
variable "github_app_id" {
  ...
  validation {
    condition     = !(var.runner_use_self_hosted && var.runner_authentication_method == "github_app") || var.github_app_id != null
    error_message = "github_app_id must be set when runner_use_self_hosted is true and runner_authentication_method is 'github_app'."
  }
}
...
variable "github_app_installation_id" {
  ...
  validation {
    condition     = !(var.runner_use_self_hosted && var.runner_authentication_method == "github_app") || var.github_app_installation_id != null
    error_message = "github_app_installation_id must be set when runner_use_self_hosted is true and runner_authentication_method is 'github_app'."
  }
}
...
variable "github_app_key" {
  ...
  validation {
    condition     = !(var.runner_use_self_hosted && var.runner_authentication_method == "github_app") || var.github_app_key != null
    error_message = "github_app_key must be set when runner_use_self_hosted is true and runner_authentication_method is 'github_app'."
  }
}
...
variable "runner_compute_type" {
  ...
  validation {
    ...
    error_message = "runner_compute_type must be either 'azure_container_app' or 'azure_container_instance'."
  }
}
...
variable "runner_container_instance_count" {
  type        = number
  default     = 4
  description = "The number of container instances to provision when `runner_compute_type` is 'azure_container_instance'. Ignored when `runner_compute_type` is 'azure_container_app'."

  validation {
    condition     = var.runner_container_instance_count >= 1
    error_message = "runner_container_instance_count must be greater than or equal to 1."
  }
}
